...
func TestNormalizeURL(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "remove scheme no path no suburl",
			inputURL: "http://boot.dev/",
			expected: "boot.dev",
		},
		{
			name:     "remove scheme no path no suburl no /",
			inputURL: "http://boot.dev",
			expected: "boot.dev",
		},
		{
			name:     "remove scheme no suburl",
			inputURL: "http://boot.dev/path/",
